...
def post_process_root(gene_ids, raw_cell_types, avg_matrix, std_matrix):
    ...
    SVG-element-ID-keyed arrays, exactly replicating the h5ad_to_id expansion
    and synthetic xylem/phloem logic from mini_app.py load_all (Genotype branch).

    Steps
    -----
    1. For each raw key in _ROOT_H5AD_TO_ID that exists in raw_cell_types,
       copy its column to every target SVG ID column.  Raw keys not in
       _ROOT_H5AD_TO_ID are silently dropped (they have no SVG representation).
    2. For each genotype prefix (shr2, scr4, col0), compute synthetic columns:
    ...
    Returns
    -------
    svg_ids    : list[str]  — 54 SVG element IDs (data_bot_id values)
    avg_matrix : np.ndarray, shape (n_genes, 54)
    std_matrix : np.ndarray, shape (n_genes, 54)
    """
    import numpy as np
    ...
    svg_ids = list(svg_col_avg.keys())
    new_avg = np.column_stack([svg_col_avg[sid] for sid in svg_ids])
    new_std = np.column_stack([svg_col_std[sid] for sid in svg_ids])

    print(
        f"  [root] Expanded {len(raw_cell_types)} raw obs groups "
        f"→ {len(svg_ids)} SVG element IDs",
        file=sys.stderr,
    )
    return svg_ids, new_avg, new_std
...
def compute_pseudobulk(h5ad_path, cell_type_col, cell_type_col2=None,
    ...
    call post_process_root() afterwards.
    ...
    masks = [cell_type_labels == ct for ct in unique_cts]
